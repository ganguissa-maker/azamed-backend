...
router.post('/:id/services', protect, structureOnly, async (req, res) => {
  try {
    const { serviceId, disponible, surRdv, prixConsultation, prixChambre, modeConsultation, infoSupplementaire } = req.body;
    ...
    const item = await prisma.hopitalService.upsert({
      ...
      update: { disponible: disponible ?? true, surRdv: surRdv ?? false, prixConsultation: prixConsultation || null, prixChambre: prixChambre || null, modeConsultation: modeConsultation || 'JOURS_OUVRABLES', infoSupplementaire: infoSupplementaire || null },
      create: { hopitalId: req.params.id, serviceId, disponible: disponible ?? true, surRdv: surRdv ?? false, prixConsultation: prixConsultation || null, prixChambre: prixChambre || null, modeConsultation: modeConsultation || 'JOURS_OUVRABLES', infoSupplementaire: infoSupplementaire || null },
      include: { service: true },
    });
...
// GET /api/hopitaux — liste publique hôpitaux vérifiés
router.get('/', async (req, res) => {
  try {
    const TYPES_HOP = ['HOPITAL_PUBLIC','POLYCLINIQUE','CLINIQUE','CABINET_MEDICAL','CABINET_SPECIALISE','CENTRE_SANTE'];
    const { search, ville, type, page = 1, limit = 15 } = req.query;
    const skip = (parseInt(page) - 1) * parseInt(limit);
    const where = { isActive:true, isVerified:true, typeStructure: { in: type ? [type] : TYPES_HOP } };
    ...
    const [data, total] = await Promise.all([
      prisma.structure.findMany({ where, skip, take: parseInt(limit), orderBy:{ nomCommercial:'asc' }, include:{ abonnements:{ orderBy:{ createdAt:'desc' }, take:1 } } }),
