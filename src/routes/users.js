// src/routes/users.js — Inscription utilisateurs corrigée (rôle PATIENT)
...
router.post('/register', [
  ...
  try {
    ...
    // ✅ CORRECTION : rôle correct selon le schema Prisma (RoleUtilisateur)
    // Valeurs valides : PATIENT, STRUCTURE, ADMIN, MEDECIN
    ...
    const user = await prisma.user.create({
      data: {
        ...
        passwordHash,
        role,
        isVerified: role === 'PATIENT', // patients auto-vérifiés
        isActive:   true,
      },
    });
    ...
    await prisma.$executeRawUnsafe(
      ...
      user.id,
      prenom || null, nom || null, ville || null, telephone || null,
  ...
  try {
    ...
    try {
      const rows = await prisma.$queryRawUnsafe(
        `SELECT * FROM "profils_utilisateurs" WHERE "userId" = $1`, user.id
      );
